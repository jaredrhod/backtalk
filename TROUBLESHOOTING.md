...
- **It records from my headset instead of the built-in mic (and my headphone audio degrades while I talk)**: backtalk follows the system default input, and the OS re-points that the moment a headset with a mic connects — which also drops a Bluetooth headset from high-quality A2DP to the narrowband HFP call profile, so the music and the agent's voice both get worse mid-sentence. Pin the mic: set `"mic_device"` in backtalk.json to the input you want, **by name** (`"MacBook Pro Microphone"`). Device indices are not stable, so the name is the contract; `python -m sounddevice` lists them. Matching is exact first, then case-insensitive substring, so `"MacBook Pro"` works too. A name that matches nothing falls back to the system default and logs which inputs it did find — the mic degrades, it never goes mute. (Not `stt_device`, which is the Whisper COMPUTE device, cpu/cuda.)
