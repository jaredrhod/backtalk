...
- **Pin the microphone if you wear a headset.** By default backtalk records from the system default input, which the OS hands to a headset the moment one connects — taking your voice down the narrowband call profile and degrading what you hear at the same time. Set `"mic_device"` in backtalk.json to the input you want by name (`"MacBook Pro Microphone"`) and the mic stays put, whatever connects for output. A name that no longer matches falls back to the default with a log line rather than failing.
